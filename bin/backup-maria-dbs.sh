...
for DATABASE in $(mysql -u "${MYSQL_USER}" -N -e 'show databases'); do
  ...
  echo "Snapshotting '${DATABASE}' and sending it to Dropbox"
  ...
  for HISTORICAL_DIR in "${HISTORICAL_DIRS[@]}"; do
    ...
    echo "Deleting historical existing ${SNAPSHOT_PATH} snapshot"
    http https://api.dropboxapi.com/2/files/delete_v2 \
      Authorization:"Bearer ${DROPBOX_TOKEN}" \
      path="${SNAPSHOT_PATH}" \
    || true

    echo "Copying current snapshot to ${SNAPSHOT_PATH}"
    http https://api.dropboxapi.com/2/files/copy_v2 \
      Authorization:"Bearer ${DROPBOX_TOKEN}" \
...
http "${HONEYBADGER_MARIA_BACKUPS_CHECKIN}"
